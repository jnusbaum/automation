...
def tempsensors(request):
    if request.method == 'POST':
        ...
        try:
            ...
            return JsonResponseBadRequest(reason="No name parameter supplied.")
        interface_id = request.POST.get('interface', default=None)
        ...
        try:
            ...
            s = TempSensor(name=sensor_name, address=address, description=description, one_wire_interface_id=interface_id)
            s.save()
            rdevices = {'count': 1, 'data': [s.as_json()]}
            return JsonResponseCreated(data=rdevices)
    else:
